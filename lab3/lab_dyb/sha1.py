...
target_prefix = hash1[:7]  # Pierwsze 7 znaków hexdigestu

found_collision = False
doc2 = None

# Szukanie drugiego dokumentu, który ma taki sam 7-znakowy prefiks SHA-1
while not found_collision:
    ...
    hash2 = hashlib.sha1(doc2.encode()).hexdigest()

    # Sprawdzanie czy 7-znakowy prefiks jest taki sam
    if hash2[:7] == target_prefix:
        found_collision = True

# Wyświetlenie wyników
print(f"Doc1: {doc1}, Hash1: {hash1}")
print(f"Doc2: {doc2}, Hash2: {hash2}")
print(f"Pierwsze 7 znaków (prefiks kolizyjny): {target_prefix}")
